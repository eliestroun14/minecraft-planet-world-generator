...
cap even perfectly centered — so `worldgen/structure/planet.json` now sets
`max_distance_from_center: 200` and genuinely relies on the mod to lift the
vanilla cap, same as the Iron Mammoth (`max_distance_from_center: 300`)
always has.
